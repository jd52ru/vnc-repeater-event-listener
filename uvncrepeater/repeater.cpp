...
//Mode 1 connection code removed - only Mode 2 supported

//This structure (and repeaterProcs[] table) is used for keeping track of child processes running doRepeater and cleaning up after they exit
    ...
    if (isExistingConnectionInactive(rI -> active, (rI -> code != 0))) {
        ...
        if (fRemove) {
            ...
            if (useEventInterface) {
                ...
                connEv.code = rI -> code;
                connEv.connMode = CONN_MODE2; // Всегда Mode 2 теперь
...
[[maybe_unused]] static bool parseHostAndPort(char *id, char *host, int hostLen, int *port)
{
    int tmpPort;
    char *colonPos;
...
// Only Mode 2 connections are accepted
static void acceptConnection(int socket, int connectionFrom)
{
    ...
    if (connection < 0)
        ...
        //id must be of format "ID:xxxxx" for Mode 2
        if (!checkIdCode(id)) {
            debug(LEVEL_2, "acceptConnection(): Only Mode 2 connections are supported, closing connection\n");
            close(connection);
            return;
        }
    ...
    if (index != UNKNOWN_REPINFO_IND) {
        ...
        if ((serverInd != UNKNOWN_REPINFO_IND) && (viewerInd != UNKNOWN_REPINFO_IND)) {
            ...
            if (useEventInterface) {
                ...
                sessEv.viewerTableIndex = viewerInd;
                sessEv.code = code;
                sessEv.connMode = CONN_MODE2; // Всегда Mode 2 теперь
                sessEv.serverIp = servers[serverInd] -> peerIp;
                sessEv.viewerIp = viewers[viewerInd] -> peerIp;
                memcpy(event.extraInfo, &sessEv, sizeof(sessionEvent));
                if (false == sendRepeaterEvent(event)) {
...
static void listInitializationValues(void)
{
    ...
    debug(LEVEL_2, "listInitializationValues(): Mode 2 connections allowed : Yes\n");
...
int main(int argc, char **argv)
{
    ...
    if (memoryOk) {
        ...
        //Initialize and start listening on viewer port (always for Mode 2)
        viewerListener.port = viewerPort;
        startListeningOnPort(&viewerListener);
        //Initialize and start listening on server port (for Mode 2)
