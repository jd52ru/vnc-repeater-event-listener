...
bool readIniFile(char *iniFilePathAndName)
{
    ...
    if (dict != NULL) {
        ...
        if (maxSessions < MAX_SESSIONS_MIN)
            maxSessions = MAX_SESSIONS_DEFAULT;
        allowedModes = iniparser_getint(dict, "general:allowedmodes", CONN_MODE2);
        ...
        strlcpy(eventListenerHost, iniparser_getstring(dict, "eventinterface:eventlistenerhost", "localhost"), MY_TMP_BUF_LEN);
